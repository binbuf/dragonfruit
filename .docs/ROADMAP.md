...
and a three-finger horizontal gesture slides the Space). Still open: the
Mission Control overview *chrome* (workspace strip, minimized-window bottom
strip, window selection/dragging between Spaces via the shell), per-surface
scale/blur/clip and the wallpaper slide (T-33), multi-monitor lockstep during
the overview, and the FR-8 60 Hz frame-time trace. Details and hand-off:
[PROGRESS.md](PROGRESS.md).
...
2. **Experience**
   ...
   - [ ] Top bar (T-09 partial: menu-bar render/interaction + shell bootstrap live, restart/idle scripts passing, chrome input routing + overlay-layer dropdown landed and scripted, output hotplug scripted, always-present system menu (dragonfruit mark) + application menu (Files on the desktop) landed; T-20 status adapters, T-22 app-exported menus, and the fixed-menu action wiring (T-16/T-24/T-26) open)
    ...
    - [ ] Mission Control (T-11 partial: the one overview state machine, trigger parity, commit rules, hit-test transfer, and selection round-trip landed; overview chrome/strip and scale/blur materials open)
    - [ ] Workspace gestures (T-11 partial: trackpad gestures drive the same pipeline as keyboard/hot corner; wallpaper slide and multi-monitor lockstep open)
   - [ ] Animations
3. **Flagship apps**
