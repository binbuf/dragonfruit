...
clock and the multi-monitor lockstep conformance test. Still open: the
per-surface scale/clip/blur *live-surface* grid and the wallpaper slide
(T-33's reusable scene-transform pass), overview hit-testing of window
representations, reduced-motion wiring, and the FR-8 60 Hz frame-time trace.
Details and hand-off: [PROGRESS.md](PROGRESS.md).
