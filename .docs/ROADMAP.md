...
[docs/private-protocols.md](../docs/private-protocols.md). Open: chrome
*rendering*, layer stacking, and keyboard-mode seat grabs are T-09/T-10
(T-07 owns placement/zones/configure only); Qt/C++ shell bindings are
generated by `scripts/gen-shell-protocol-bindings.sh` but not yet consumed
until T-08; VRR/night-light output requests are acked but not plumbed
(T-16); reserved zones are a single global union rather than per-output
(T-11/T-16); toplevel thumbnails for the Dock are deliberately absent
(token-gated if added, T-10); the additive-only proof test (FR-7) is
