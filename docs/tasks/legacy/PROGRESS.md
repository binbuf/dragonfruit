...
> **Plan note (2026-09-23).** The phase-based ticket plan was re-cut into a
> sequential vertical-slice plan: see [ROADMAP.md](../../ROADMAP.md).
> Everything below is historical record from the legacy tickets; references
> to `tasks/NN-*.md` mean `docs/tasks/legacy/NN-*.md`, and T-xx numbers in
> this file are legacy numbers (the new plan restarts at T-01).
...
[14-risks.md](../../design/14-risks.md) ("reproduce the interaction
...
## T-11 — unblocked items: reduced motion, frame timing, trigger/hit-test conformance, layout spec (fourth slice)

**State: the T-11 "remaining work — unblocked" list is largely closed.** U-1,
U-2, U-5, U-6, U-7 and U-8 landed; U-4 landed its coverage rule (per-output
sizing stays on the T-16 seam). U-3 (live-surface video conformance) is still
open because it needs B-1's scale pass. `cargo test -p
dragonfruit-compositor --bin dragonfruit-compositor` 127/127, the full
`shell_protocol_conformance` 30/30, `make e2e` green, `ctest` 15/15, clippy
`-D warnings` and fmt clean.

### Protocol: `df_toplevel_manager` is now v3 (`set_reduced_motion`)

- **Declare a version-gated request *after* the events, not before
  `destroy`.** `wayland-scanner` (the C scanner used for the Qt bindings)
  warns "since version not increasing" unless every message in the interface
  has a non-decreasing explicit `since`; a `since="3"` request followed by
  v1 events/destroy trips it. Putting the request last keeps the warning away
  *and* preserves every existing message opcode (append-only), unlike the
  earlier `release_keyboard_focus` insertion which shifted `destroy`.
- `MANAGER_INTERFACE_VERSION = 3` in `compositor/src/shell/mod.rs`; the shell
  binds `min(m_managerVersion, 3u)` in `shellprotocol.cpp`. Qt bindings are
  regenerated by the CMake build from the XML; the Rust server bindings are
  generated by the `wayland_scanner` proc macro, which does **not** always
  re-run on an XML-only change — `touch compositor/src/shell_protocol/mod.rs`
  (or a clean) forces it. Watch for this: a stale server binding and a fresh
  C binding would disagree on opcodes.

### The one machine (U-7): directional shell requests

- `OverviewMachine::drive_overview(active, trigger, now)` sets a
  `requested_overview: Option<bool>` that `commit_from_events` uses instead of
  the toggle (`!overview_active`); the field is cleared on commit, cancel,
  gesture begin, `set_overview`, and `select`. `drive` keeps the toggle
  behavior (keyboard/hot corner/gesture).
- `DfState::drive_overview_request(active, serial)` records the shared
  `InputAction::MissionControl`/`TriggerKind::Shell` action, drives the
  machine, pushes `progress` events, and arms the same 16 ms animation clock.
  `enter_mission_control`/`exit_mission_control` call it; `DfState::set_overview`
  is now only the selection round-trip/`T-12` seam.
- The private-protocol request is **not** applied synchronously anymore, so
  conformance tests must wait for `overview_changed` (the 200 ms clock commits
  quickly); the existing round-trip test already did.

### Frame-time measurement (U-2)

- `RenderStats` is **no longer `Copy`** (it now holds a bounded `VecDeque` of
  `FrameSample { duration_us, interval_ms }`, a total/max, and a
  `last_frame_at`). `RenderStats::new()` reads `DRAGONFRUIT_FRAME_TRACE` once.
- `session.rs` times each loop's `render` call and records a sample only when
  `frames_rendered` advanced, so the interval is the animation-clock interval
  between *rendered* frames (idle gaps are not sampled).
- `dump_stats` (SIGUSR1 + clean exit) always prints a `frame timing` summary
  line and, when `DRAGONFRUIT_FRAME_TRACE` is set, `frame-trace` lines. The
  headless `frame-trace` records ~one sample per rendered frame; the
  `overview_gesture_frame_trace_stays_within_budget` test asserts samples > 0
  and `max_interval_ms <= 100` (generous for CI). The hardware 60 Hz budget
  is B-5; scale/blur degrade tiers are B-6.
- The conformance harness now redirects the compositor's **stdout to a temp
  file** (was `Stdio::null()`), with `read_stdout()`/`dump_and_read_stdout()`
  helpers and a `start_with_synthetic_trace` constructor. SIGUSR1 is 10.

### Reduced motion (U-1/U-6)

- Shell side: `ShellController::applyDockSettings` calls
  `ShellProtocol::setReducedMotion(m_settings.reduceMotion())` next to the
  `Theme.reducedMotion` singleton write. `Overview.qml` derives
  `revealProgress` from `Theme.reducedMotion` (instant appearance, no slide)
  for the scrim/strips/grid; `tst_overview` covers it.
- `reduced_motion_single_steps_every_transition_kind` covers every
  `OverviewKind`; `reduced_motion_request_single_steps_the_overview` asserts
  the end-to-end request yields `progress_events <= 3` (Begin/Update/End).

### Per-output coverage (U-4) and the layout spec (U-5)

- **A full-output `overlay` (anchored on all four edges) now spans every
  output** (`LayerSurfaceState::is_full_output` short-circuits
  `visible_on_output`), so Mission Control is composited on every display even
  when chrome focus was captured on one. Per-output popovers (fewer anchors)
  keep their T-10 behavior.
- **Per-output *sizing* is still open**: `configure_layer` resolves a
  null-output surface to the *first* output's geometry, and the shell renders
  one offscreen scene sized from that configure, so a mixed-size multi-monitor
  overview is only correct on same-size displays. One `df_layer_surface` +
...
- **A four-finger *down* swipe is Desktop Reveal, not Mission Control close.**
  A frame-time test that opened the overview with an up-swipe then tried to
  close it with a down-swipe never saw `active == 0`; close through
  `exit_mission_control` (or another up-swipe when already open, which the
  toggle handles) instead.
- **The U-8 test relies on `surface_under` returning `None` while the overview
  owns input**, which makes Smithay send a pointer `leave` on the next motion;
  assert on `pointer_leaves` increasing and `pointer_enters` staying flat.
- `set_overview` in `DfState` is kept for the selection round-trip; if it
  becomes truly unused, drop it rather than letting `dead_code` creep back.

### Gate status

`cargo test -p dragonfruit-compositor --bin dragonfruit-compositor` 127/127
(incl. the new `shell_request_drives_directionally...`,
`reduced_motion_single_steps_every_transition_kind`, and
`full_output_overlays_span_every_output`); `shell_protocol_conformance` 30/30
(incl. the four new T-11 tests); `make e2e` green; `ctest` 15/15 (incl.
