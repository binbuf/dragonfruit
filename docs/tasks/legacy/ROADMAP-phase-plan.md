# Roadmap and MVP Definition (phase-plan history)

> **Historical record.** This is the pre-re-cut, phase-based roadmap. The
> current plan is [../../ROADMAP.md](../../ROADMAP.md) (strict, single-session
> work units). Kept for the MVP definition and the per-ticket narratives.
...
> **Plan status (2026-09-23).** The phase-based ticket sequence in this file
> has been re-cut into a sequential vertical-slice plan: see
> [ROADMAP.md](../../ROADMAP.md). The slice plan supersedes the
> "Ticket status" and "Revised delivery order" sections below; the
> per-ticket narratives further down are kept as historical record. The
> design docs remain the source of truth for *what* to build.
...
### Vertical-slice sequence (current plan)

The current plan, the slice demos, the definition of done, and the legacy
mapping live in [ROADMAP.md](../../ROADMAP.md). In brief:

| Slice | Demo |
|---|---|
| T-01 Loop v0 — window controls | launch → titled window → drag/zoom/minimize/restore/close |
| T-02 Loop v1 — lifecycle motion | the same loop with appear/minimize/restore/zoom/close motion |
| T-03 Real session — bring-up & perf | the loop on DRM/logind, budgets measured |
| T-04 Loop v2 — materials | blur, shadows, rounded corners |
| T-05 Loop v3 — Mission Control live | live-surface overview, wallpaper slide, Desktop Reveal |
| T-06 Loop v4 — app switcher | Cmd+Tab with live previews |
| T-07 Menu bar live | real Wi-Fi, volume, battery |
| T-08 settingsd | one owner for desktop settings, live signals |
| T-09 Settings Wave 1 | Appearance, Wallpaper, Desktop & Dock, Displays-basic |
| T-10 Files MVP | browse, open, rename, trash; Dock↔Files Trash |
| T-11 Control Center + notifications | panel, banners, OSD |
| T-12 Session + lock + idle | real login session, lock, idle, suspend |
| T-13 Portals + capture + clipboard | Flatpak browser walkthrough |
| T-14 Global menu + app index + compat | real menus, tray, DBusMenu, XDnD, zoo |
| T-15 System services + Settings Waves 2–3 | Bluetooth, storage, printers, users, … |
| T-16 Platform polish + packaging | multi-monitor, scaling, suspend soak, a11y, i18n, packages |
| T-17 Premium gate | the full 30-second loop, nested + DRM, visual floor |

The legacy phase taxonomy and per-ticket narratives are kept below as
historical record; the legacy ticket files are archived in
[tasks/legacy/](./).

### Legacy ticket history (historical record)
