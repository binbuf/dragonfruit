...
the selection via settingsd keys; the shell forwards it (ADR 0038). The
`source` is a local path in every case; since T-18 the shell may resolve it
from the fetched Wikimedia provider (a user choice in `wallpaper.source` wins
over the provider default), and this compositor path is otherwise unchanged
(ADR 0055).
